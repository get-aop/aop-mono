...
export const reconcileRepo = async (repo: Repo, deps: ReconcileDeps): Promise<ReconcileResult> => {
  ...
  const existingTasks = await deps.taskRepository.list({ repo_id: repo.id });
...
const createMissingTasks = async (
  ...
  const activeTaskPaths = new Set(
    existingTasks.filter((t) => t.status !== "REMOVED").map((t) => t.change_path),
  );

  let created = 0;
  for (const changeName of changesOnDisk) {
    // Store relative path from repo root (e.g., "openspec/changes/mychange")
...
const removeOrphanedTasks = async (
  ...
  // Build set of relative paths (e.g., "openspec/changes/mychange")
