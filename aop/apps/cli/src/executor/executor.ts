...
import { detectSignal } from "../sync/signal-detector.ts";
import { createTemplateContext, resolveTemplate } from "../sync/template-resolver.ts";
...
export const executeTask = async (
  ...
  while (true) {
    const { executionId, stepId } = await createExecutionRecords(ctx, task.id);
