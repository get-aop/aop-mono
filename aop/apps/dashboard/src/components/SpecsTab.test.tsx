...
describe("SpecsTab", () => {
  test("loads tasks.md as default file", async () => {
    setupMockFetch(["tasks.md", "design.md"], {
      "tasks.md": "# Tasks\n- [x] Done\n- [ ] Pending",
    });
  ...
  test("shows file tree flyout", async () => {
    setupMockFetch(["tasks.md"], { "tasks.md": "# Tasks" });
    render(<SpecsTab task={makeTask()} />);
    await waitFor(() => expect(screen.getByTestId("file-tree-flyout")).toBeTruthy());
  });

  test("shows loading state initially", () => {
    setupMockFetch(["tasks.md"], { "tasks.md": "content" });
    render(<SpecsTab task={makeTask()} />);
  ...
  test("shows error on fetch failure", async () => {
    mockFetch.mockImplementation((...args: unknown[]) => {
      ...
      if (urlStr.endsWith("/files")) {
        return Promise.resolve(jsonResponse({ files: ["tasks.md"] }));
      }
      return Promise.resolve(jsonResponse({ error: "Not found" }, 404));
    });
    render(<SpecsTab task={makeTask()} />);
