...
This creates a DMG installer in `out/make/`. Open the DMG and drag AOP to Applications.
...
This builds the sidecar, dashboard, packages for Windows, and creates `out/AOP-Desktop-Windows-Latest.zip`.

To run: unzip and double-click `aop.exe`.
...
```bash
cd out/AOP-linux-x64
./aop --no-sandbox
```

## Distribution

Send users the appropriate file for their platform:

| Platform | File | Location | How to run |
|----------|------|----------|------------|
| macOS | `.dmg` | `out/make/` | Open DMG, drag AOP to Applications, launch from Applications |
| Windows | `.zip` | `out/AOP-Desktop-Windows-Latest.zip` | Unzip, double-click `aop.exe` |
| Linux/WSL | `.deb` | `out/make/` | `sudo dpkg -i aop_*.deb`, then run `aop --no-sandbox` |
