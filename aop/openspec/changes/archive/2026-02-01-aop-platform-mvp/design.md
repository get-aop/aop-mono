...
**Choice**: Heartbeat-based health monitoring with state reconciliation on reconnect.

**Heartbeat:**
```
CLI → Remote: ping (every 30s)
Remote → CLI: pong

No pong in 90s → connection dead, trigger reconnect
No ping in 90s → remote marks CLI as disconnected
```

**Reconnection:**
```
On disconnect:
  1. Queue local state changes (step completed, task status)
  2. Running agents continue locally (independent of connection)
  3. Exponential backoff: 1s → 2s → 4s → 8s → max 60s

On reconnect:
  1. CLI sends full task state snapshot
  2. Remote sends full execution state snapshot
  3. Reconcile using ownership rules (Decision 10)
  4. Flush queued state changes
  5. Resume workflows (remote sends next step commands)
```

**Mid-Workflow Disconnect:**
- Agent continues running locally until step completes
- Completed step is queued, but next step CANNOT start (remote has the prompt)
- Workflow resumes automatically when connection restores
- No manual intervention required

**Idempotency:**
- All messages include (execution_id, step_id, attempt)
- Remote deduplicates repeated messages
- Safe to retry any message

**Rationale**: Simple state reconciliation is more robust than guaranteed message delivery for this use case. Agents are the slow part (minutes); connection recovery is fast (seconds).
  ...
  cli/                        # CLI + local daemon
    src/
      ...
      commands/               # CLI commands
        init.ts               # aop init
        start.ts              # aop start (daemon mode)
        stop.ts               # aop stop
...
- `apps/server/src/api`: WebSocket server
- `apps/cli/src/sync`: WebSocket client, state sync
...
**[Workflow paused when offline]** → If connection drops mid-workflow, completed steps are queued but next step cannot start until remote provides the command. Workflow resumes automatically on reconnect. This is an accepted trade-off of the remote workflow engine design.
