...
describe("CursorCliProvider", () => {
  test("implements LLMProvider interface", () => {
    const provider: LLMProvider = new CursorCliProvider("Composer 1.5");
    expect(provider.name).toBe("cursor-cli");
    expect(typeof provider.run).toBe("function");
  });

  test("has readonly name property", () => {
    const provider = new CursorCliProvider("Composer 1.5");
    expect(provider.name).toBe("cursor-cli");
  });

  test("stores the model", () => {
    const provider = new CursorCliProvider("Composer 1.5");
    expect(provider.model).toBe("Composer 1.5");
  });
});

describe("buildCommand", () => {
  test("builds command for Composer 1.5", () => {
    const provider = new CursorCliProvider("Composer 1.5");
