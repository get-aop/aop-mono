...
describe("formatSubtaskCompletedMessage", () => {
  test("formats subtask completion with duration", () => {
    const result = formatSubtaskCompletedMessage(1, 7, "Create dashboard layout", 363000);
    expect(result).toBe("✓ Subtask 1/7: Create dashboard layout completed (6m 3s)");
  });

  test("formats with hour-long duration", () => {
    const result = formatSubtaskCompletedMessage(2, 5, "Implement API", 5520000);
