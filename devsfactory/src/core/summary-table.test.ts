...
describe("renderSummaryTable", () => {
  const createSummary = (overrides: Partial<TaskSummary> = {}): TaskSummary => ({
  ...
  test("truncates long subtask titles with ellipsis", () => {
    ...
    expect(output).toContain("…");
    expect(output).not.toContain("This is a very long subtask title that should be truncated");
  });
  ...
  test("right-aligns duration columns", () => {
    ...
    const dataLines = lines.filter(line => line.includes("Short task") || line.includes("Longer task"));
