/**
 * @deprecated Migration-only module for parsing plan.md files from legacy filesystem storage.
 * Used by the `aop migrate` command and transitional sync code.
 * Do not use for new code - all new plan data should be read from SQLiteTaskStorage.
 */
...
export const parsePlan = async (
  ...
  try {
    ...
    // Plan files without proper frontmatter are treated as non-existent
    // This allows for plain markdown plan files created by Claude
    if (error instanceof ZodError) {
      throw new ParseError(filePath, error);
    }
    // Missing frontmatter - treat as no plan
    return null;
  }
