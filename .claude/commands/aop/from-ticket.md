...
2. For Linear, resolve refs, URLs, ranges, and mixed lists into the full issue set before writing files.
3. If a required Linear blocker is missing locally, import it as a draft dependency task in the same repo.
4. Verify the source against the codebase and fix stale assumptions in the plan.
5. Derive a kebab-case task slug from the source title for each imported task.
6. Create `docs/tasks/<task-slug>/` if it does not exist.
7. Write `docs/tasks/<task-slug>/task.md` with the extracted requirements and acceptance criteria.
8. Write `docs/tasks/<task-slug>/plan.md` with the implementation checklist, context, and verification steps.
9. Add numbered subtask files under `docs/tasks/<task-slug>/` when the work needs multiple executable slices.
10. Report which tasks came from the requested input and which were auto-imported as blockers.
11. Ask whether the imported tasks should be started now.
12. If the answer is yes, invoke the `task-ready` skill only for the imported tasks that should move to `READY`.
13. Explain that some started tasks may remain `READY` until their dependency tasks are `DONE`.
14. Present the task slug(s), written files, final status, and a short summary for review.
